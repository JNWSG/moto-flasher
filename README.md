### Run adb and fastboot in Termux without root permissions!

**official** [Repo](https://github.com/nohajc/termux-adb) **by:** [nohajc](https://github.com/nohajc)

**modified** [Repo](https://github.com/offici5l/termux-adb-fastboot) **by** [offici5l](https://github.com/offici5l)

### Required APK

Install [Termux](https://f-droid.org/en/packages/com.termux/) & [Termux-api](https://f-droid.org/en/packages/com.termux.api/)

Download This apk & install & run Termux then 

### Required Package

```bash
pkg install git -y
```
### Script Cloning & Running Command 

```bash
git clone https://github.com/JNWSG/moto-flasher.git
```

```bash
cd moto-flasher
```

```bash
chmod +x moto-flasher.sh
```

```
./moto-flasher.sh
```
